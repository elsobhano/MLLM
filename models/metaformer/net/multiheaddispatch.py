...
from models.metaformer.net.rotary_embeddings import RotaryEmbedding
...
class MultiHeadDispatch(nn.Module):
    ...
    def forward(
        ...
        # value = value.masked_fill(~att_mask.unsqueeze(-1), 0)
        ...
        #     .expand(att_mask.shape[0], self.num_heads, att_mask.shape[1])
        #     .reshape(att_mask.shape[0] * self.num_heads, att_mask.shape[1])
        # )
        # v = v.masked_fill(~am.unsqueeze(-1), 0)
        # Self-attend
        # print(q.std(), k.std(), v.std())
        # print(self.num_heads, self.dim_key_head, self.dim_value_head,self.dim_model)
        ...
        # y = y.masked_fill(~att_mask.unsqueeze(-1), 0)
