...
class FeatureExtracter(nn.Module):
    ...
    def forward(self,
        ...
        hamer_2 = self.hamer_mapper_2(hamer_1_activated)
        hamer_2_dropped = self.hamer_mapper_1_drop(hamer_2)
        src = torch.cat([src, hamer_2_dropped], dim=-1)
        ...
        desc_1_dropped = self.mapper_1_drop(desc_2)

        src = torch.cat([src, desc_1_dropped], dim=-1) #(batch_size, new_seq_len, new_dim=1024)

        return src, mask, hamer_1_activated, desc_1_activated
